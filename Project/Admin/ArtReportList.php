			...
			<?php
			 ...
			 $selQry = "select * from tbl_genere order by genere_name";
				 ...
				 $rs = $Conn->query($selQry);
				   ...
				   {
					   $selQry3 = "select COALESCE(count(art_id),0) as stock from tbl_art where genere_id='".$data["genere_id"]."'";
					   
					   
					   $rs3 = $Conn->query($selQry3);
						$data3 = $rs3->fetch_assoc();
					   
					   $selQry2 = "select COALESCE(count(b.art_id),0) as qty from tbl_artbooking b inner join tbl_art c on c.art_id=b.art_id where genere_id='".$data["genere_id"]."' ";
