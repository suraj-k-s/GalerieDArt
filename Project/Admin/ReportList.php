    ...
    <body>
        ...
        <table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
			...
			<?php
				...
				$i=0;
			 $selQry = "select * from tbl_product order by product_name";
				 ...
				 $rs = $Conn->query($selQry);
				   ...
				   {
					   $selQry3 = "select COALESCE(SUM(stock_qty),0) as stock from tbl_stock where product_id='".$data["product_id"]."'";
					   
					   
					   $rs3 = $Conn->query($selQry3);
						$data3 = $rs3->fetch_assoc();
					   
					   $selQry2 = "select COALESCE(SUM(cart_qty),0) as qty from tbl_booking b inner join tbl_cart c on c.booking_id=b.booking_id where product_id='".$data["product_id"]."' and  cart_status>0 ";
					   
					   if(isset($_POST["btn_search"]))
						{
							$selQry2 = $selQry2." and booking_date between '".$_POST["fdate"]."' and '".$_POST["tdate"]."'";
						}
						
						$rs2 = $Conn->query($selQry2);
			...
			?>
              <tr>
                 ...
                 <td align="center"><?php echo $data3["stock"]; ?></td>
